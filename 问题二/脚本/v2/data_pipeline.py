...
def load_bundle(force_rebuild: bool = False) -> DataBundle:
    ...
    if not force_rebuild and _cache_is_valid():
        ...
        _write_csv(
            pd.DataFrame(
                [
                    {"检查项": "附件二行数", "实际值": bundle.audit["附件二行数"]},
                    {"检查项": "日期—单品批发价匹配率", "实际值": bundle.audit["日期—单品批发价匹配率"]},
                    {"检查项": "全量品类日期面板行数", "实际值": bundle.audit["全量品类日期面板行数"]},
                    {"检查项": "正常销售品类日期面板行数", "实际值": bundle.audit["正常销售品类日期面板行数"]},
                ]
            ),
            "01_数据审计.csv",
        )
        cached_audit_rows = [
            {"检查项": "附件一行数", "实际值": bundle.audit["附件一行数"], "是否通过": "是" if bundle.audit["附件一行数"] == 251 else "否"},
            {"检查项": "附件二行数", "实际值": bundle.audit["附件二行数"], "是否通过": "是" if bundle.audit["附件二行数"] == 878503 else "否"},
            {"检查项": "附件三行数", "实际值": bundle.audit["附件三行数"], "是否通过": "是" if bundle.audit["附件三行数"] == 55982 else "否"},
            {"检查项": "缺失单元格总数", "实际值": sum(bundle.audit["缺失单元格总数"].values()), "是否通过": "是" if sum(bundle.audit["缺失单元格总数"].values()) == 0 else "否"},
            {"检查项": "日期—单品批发价匹配率", "实际值": bundle.audit["日期—单品批发价匹配率"], "是否通过": "是" if bundle.audit["日期—单品批发价匹配率"] == 1 else "否"},
            {"检查项": "销售量符号不一致数", "实际值": bundle.audit["销售量符号不一致数"], "是否通过": "是" if bundle.audit["销售量符号不一致数"] == 0 else "否"},
            {"检查项": "全店无流水日期数", "实际值": bundle.audit["无流水日期数"], "是否通过": "是"},
        ]
